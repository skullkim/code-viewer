...
struct TextSearchPresentationTests {
    ...
    private func result(
        _ items: [TextSearchItem],
        total: Int? = nil,
        truncated: Bool = false,
        filesSearched: Int = 0
    ) -> TextSearchResult {
        TextSearchResult(
            items: items, total: total ?? items.count, truncated: truncated,
            limit: 500, filesSearched: filesSearched
        )
    }

    @Test("검색 전에는 입력 안내만 보인다")
    func theIdleStateInvitesAQuery() {
        let panel = TextSearchPresentation.make(phase: .idle, previousResult: nil, elapsedSeconds: nil)
    ...
    func resultsCarryTheirMetadata() {
        let panel = TextSearchPresentation.make(
            phase: .results(result([item("a.swift", 1), item("a.swift", 9), item("b.swift", 3)], filesSearched: 1284)),
            previousResult: nil, elapsedSeconds: 0.42
        )
    ...
    @Test("훑은 파일 수는 결과가 보고한 값을 그대로 쓴다")
    func theFileCountComesFromTheSearchItself() {
        // On a truncated search this is smaller than the project's file count. Substituting
        // anything else — the project total, the number of files with hits — would state
        // something the search never did.
        let panel = TextSearchPresentation.make(
            phase: .results(result([item("a.swift", 1)], truncated: true, filesSearched: 37)),
            previousResult: nil, elapsedSeconds: 0.1
        )
        #expect(panel.metaText == "1건 표시 · 37 파일 검색 · 0.10초")
    }

    @Test("결과가 0건이면 결과 없음을 표시한다")
    func anEmptyResultSaysSo() {
        let panel = TextSearchPresentation.make(phase: .results(result([])), previousResult: nil, elapsedSeconds: 0.1)
    ...
    func hittingTheCapIsAnnounced() {
        ...
        let panel = TextSearchPresentation.make(
            ...
            previousResult: nil, elapsedSeconds: 1.0
        )
    ...
    func noCapNoWarning() {
        let panel = TextSearchPresentation.make(
            phase: .results(result([item("a.swift", 1)])), previousResult: nil, elapsedSeconds: 0.1
        )
    ...
    func anInvalidRegexIsAnErrorNotAnEmptyResult() {
        ...
        let panel = TextSearchPresentation.make(
            ...
            previousResult: previous, elapsedSeconds: nil
        )
    ...
    func aRegexErrorWithNoHistoryIsStillAnError() {
        let panel = TextSearchPresentation.make(
            phase: .failed(.invalidRegularExpression(pattern: "([", reason: "미완성")),
            previousResult: nil, elapsedSeconds: nil
        )
        #expect(panel.errorText != nil)
        #expect(panel.items.isEmpty)
    ...
    func otherFailuresAreShownToo() {
        let panel = TextSearchPresentation.make(
            phase: .failed(.noProjectOpen), previousResult: nil, elapsedSeconds: nil
        )
    ...
    func aRunningSearchKeepsThePreviousResultsReadable() {
        ...
        let panel = TextSearchPresentation.make(phase: .searching, previousResult: previous, elapsedSeconds: nil)
