...
1. `./scripts/bundle.sh` — `.app` 조립이 성공하는가 (plist의 `CFBundleExecutable`이 실제 실행 파일과 일치하는지도 검사)
2. `./scripts/verify-bundle.sh` — 조립된 앱을 **실제로 실행**해 `--self-check`가 번들 식별자를 되보고하는지 확인
   → **디렉토리가 생겼다는 것은 동작의 증거가 아니다.** 존재가 아니라 실행을 확인한다.
3. `./scripts/verify-bundle.sh --self-test` — **검사기 자체 검사**. 틀린 식별자·실행 파일 부재를 실제로
   잡는지, 정상 번들은 오탐 없이 통과하는지 양방향으로 확인한다.

스크립트는 대상 앱이 없는 상태에서 **spike 바이너리로 4방향 실측을 마쳤다**(2026-08-29 17:29):
틀린 식별자 검출 · 실행 파일 부재 검출 · plist 실행파일명 불일치 검출 · 정상 통과.
앱 타깃이 생기면 기본값(`CodeNavigator`)으로 그대로 동작한다.

**앱은 `--self-check` 플래그를 지원해야 한다** — 번들 식별자와 실행 파일 이름을 표준 출력에 쓰고
종료한다. 게이트가 윈도우 서버 세션 없이도 돌 수 있어야 하기 때문이다.
