...
| **REQ-014 2단계 · D-13 (삽입 모드 한글 조합)** | frontend | — | frontend-senior | 🔴 **미착수.** `NSTextInputClient` 10메서드 + `keyDown` 분기. **조합 중 표시가 미지수** — 커밋 전 글자는 Neovim 버퍼에 없어 그리드가 못 그린다. (a)커밋 시에만 전송 / (b)marked text 오버레이 중 리더 판정 대기 |
