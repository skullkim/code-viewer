...
### 3.5 다중 프로젝트 탭 (REQ-012) — 본문은 `03c`

**본문 단일 소스: `_workspace/03c_multi_project_contract_draft.md`. 상태: 채택됨(리더 승인).**
ADR과 같은 방식으로 링크만 둔다 — 여기에 옮겨 적으면 본문이 둘이 되고, 둘이 된 순간 어느 쪽이
계약인지 아무도 모른다.

프론트엔드가 확인 요청한 4건은 전부 그 문서에 이미 답이 있다:

| 질문 | 답 | 위치 |
|---|---|---|
| `showtabline=0` | **설정한다.** 앱이 탭 바를 그리므로 Neovim 탭줄과 겹치면 그리드 행이 한 줄 밀린다. **탭 1개로는 재현되지 않는다**(기본값 1, 2개 이상에서 발현) — INV-4 위반이 아니다(UI 렌더 계약이지 사용자 편집 설정이 아니다) | 03c §3 |
| `TabEnter` 통지 | **필요하다. 활성 탭의 단일 소스는 Neovim이다.** `gt`·`:tabnext`를 앱이 모르면 화면과 탭 바가 어긋난다. autocmd → `rpcnotify` → `WorkspaceEvent.activeTabChanged` | 03c §1·§3 |
| 마지막 탭 닫기 | **세션을 유지하고 빈 탭페이지로 둔다.** `tabclose`는 마지막 탭페이지에서 `E784`로 실패하고 프로세스는 산다(실측) | 03c §8.1 |
| 추가 개방 API + 경로 정규화 | `openProject(at:) -> ProjectOpenOutcome`(`opened`/`activatedExisting`로 가른다 — 불린이면 앱이 추측하게 된다). **정규화는 엔진이 소유**하고 앱은 그 값을 키로 쓴다. 앱이 자기 규칙으로 정규화하면 같은 프로젝트가 두 탭이 된다 | 03c §2·§4 |
