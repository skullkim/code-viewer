...
# ---------------------------------------------------------------------------
# 스텝이 무엇을 읽는가 — 판정의 입력 목록
# ---------------------------------------------------------------------------
# 게이트에 검사를 넣을 때는 **그 검사가 읽는 파일이 전부 커밋되는 영역인지** 함께 본다.
# 한 번 당했다: `check-design-tokens.sh` 를 게이트에 넣었는데 그것이 읽는 `02_design.md` 가
# 미커밋이었다. 그러면 같은 HEAD 에서 사람마다 다른 판정이 나오고, 그 차이는 "게이트가
# 불안정하다"로 읽힌다 — 실제로는 게이트가 트리 밖을 보고 있었던 것이다.
#
#   스텝                        읽는 것
#   ─────────────────────────────────────────────────────────────────────────
#   swift build / swift test    Sources/ · Tests/ · Package.swift        (커밋됨)
#   실행 테스트 수              위 실행의 출력                            (파생)
#   시각 회귀                   Tests/ 의 DesignRegression 스위트         (커밋됨)
#   편집기 색 토큰 정합         Sources/ 의 토큰·팔레트·nvim 스크립트     (커밋됨)
#   화면 뷰 마운트              Sources/CodeNavigatorAppKit/View/         (커밋됨)
#   시스템 입력 소스 비침범     Sources/ 전체 (TIS 호출 부재를 본다)      (커밋됨)
#   숫자 형식 단일 소스         Sources/                                  (커밋됨)
#   검색 결과의 탭 범위         Sources/                                  (커밋됨)
#   포커스 표면 대칭            Sources/CodeNavigatorAppKit/              (커밋됨)
#   .app 조립 / 번들 실행       Resources/Info.plist · Resources/treesitter · Resources/nvim
#                               (앞의 둘은 커밋, `Resources/nvim` 은 스크립트가 받아 조립 —
#                                없으면 조립이 실패하므로 조용히 통과하지 않는다)
#   유휴/탭 메모리              위 테스트 실행 (격리)                     (파생)
#   고아 Neovim 프로세스        살아 있는 프로세스 목록                   (환경)
#   민감정보 스캔               Sources/ · Tests/ · scripts/ · _workspace/ (혼합 — 아래 주석 참조)
#
# `_workspace/` 는 커밋되는 영역이지만 빌드 중에 팀이 쓰는 곳이기도 하다. 그 안을 읽는
# 검사는 "지금 누가 편집 중인 파일"을 볼 수 있으므로, 판정에 쓰기 전에 그 파일이 안정된
# 상태인지 확인한다.
...
section "프론트엔드: 편집기 색 토큰 정합"
# §4.1.1(디자인) · 프로토타입 CSS · Swift 토큰 — 같은 색을 적는 곳이 셋이다. 셋이 갈라지면
# 화면·기준 스크린샷·코드가 서로 다른 색을 주장하고, 디자인 충실도 대조가 무엇과 맞춰야
# 하는지 알 수 없게 된다. 실제로 구문 색이 산문(§4.1)과 표(§4.1.1) 두 곳에 살면서 값 5개가
# 갈라진 적이 있고, 그때 구현은 덜 권위 있는 쪽을 읽고 있었다.
#
# ⚠ 이 검사는 **값의 일치만** 본다. 팔레트가 그 토큰을 실제로 쓰는지는 안 본다 — 토큰을
# 선언만 하고 아무도 안 쓰면 여기는 초록이다. 그 층은 `SyntaxPaletteBuilderTests` 가
# 맡는다(어느 토큰이 어느 슬롯으로 가는지 + 옛 토큰과 같지 않은지 양방향).
# 검사기 하나가 전부를 답하는 척하지 않게 두 문장을 여기 적어 둔다.
if TOKEN_OUTPUT="$(bash "$REPO_ROOT/_workspace/check-design-tokens.sh" 2>&1)"; then
    pass "편집기 색 토큰 — ${TOKEN_OUTPUT##*ok: }"
else
    fail "편집기 색 토큰 정합"
    printf '%s\n' "$TOKEN_OUTPUT" | sed 's/^/    /'
fi
