...
> 1차 범위(연결·중단·관찰·해제)가 화면에서 끝까지 돈다. 인증 근거는
> `_workspace/DEBUGGER_COMPLETE.md` 에 있다 — 무엇을 어떤 방법으로 쟀는지까지 적혀 있다.
>
> 2차(스텝·예외 브레이크포인트·거터 클릭·멈춘 줄 강조 등)를 시작할 때 이 줄을
> `진행 중` 으로 되돌리면 SessionStart 훅이 다시 이 문서를 주입한다.
